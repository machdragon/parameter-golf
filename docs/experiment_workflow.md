...
python3 scripts/extract_run_metrics.py runs/*/*/train.log
```

`run_wave1_screen.sh` tees trainer output to each run’s `train.log` (not under `logs/`). `extract_run_metrics.py` also accepts plain log paths or `logs/<RUN_ID>.txt` from a direct `train_gpt.py` invocation.
