...
class Hyperparameters:
    ...
    lawa_ema_decay = float(os.environ.get("LAWA_EMA_DECAY", "0.995"))
...
def main() -> None:
    ...
    # LAWA-EMA: float32 shadow for exponential moving average
    lawa_shadow: dict[str, Tensor] | None = None
    if args.lawa_enabled:
        lawa_shadow = {k: v.detach().float().clone() for k, v in base_model.state_dict().items() if v.is_floating_point()}
        log0(f"lawa:init decay={args.lawa_ema_decay} shadow_params={len(lawa_shadow)}")
    ...
    while True:
        ...
        # LAWA-EMA: update shadow after each optimizer step
        if lawa_shadow is not None:
            with torch.no_grad():
                for k, t in base_model.state_dict().items():
                    if t.is_floating_point() and k in lawa_shadow:
                        lawa_shadow[k].mul_(args.lawa_ema_decay).add_(t.detach().float(), alpha=1 - args.lawa_ema_decay)
    ...
    # Apply LAWA-EMA shadow weights before serialization
    if lawa_shadow is not None:
        log0("lawa:applying EMA shadow weights to base_model")
        with torch.no_grad():
            for k, t in base_model.state_dict().items():
                if k in lawa_shadow and t.is_floating_point():
                    t.copy_(lawa_shadow[k].to(device=t.device, dtype=t.dtype))
        if distributed:
