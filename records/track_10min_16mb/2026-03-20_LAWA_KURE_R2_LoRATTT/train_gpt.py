"""
train_gpt_submit.py — Submission v3: KURE/R2 quant reg + tanh reparam + parallel EMA +
causal LoRA TTT, on top of v2 (int6 + LAWA-EMA + Overtone + MLP3x).
"""
...
class Hyperparameters:
    ...
    kure_lambda = float(os.environ.get("KURE_LAMBDA", "0.01"))
    r2_lambda = float(os.environ.get("R2_LAMBDA", "0.01"))

    # Tanh reparameterization: store latent P, use tanh(P) in forward
    tanh_reparam = bool(int(os.environ.get("TANH_REPARAM", "1")))

    # LoRA test-time training at eval
    ttt_lora_rank = int(os.environ.get("TTT_LORA_RANK", 8))
    ttt_lora_lr = float(os.environ.get("TTT_LORA_LR", 0.01))
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", 256))
    ttt_eval_seq_len = int(os.environ.get("TTT_EVAL_SEQ_LEN", 1024))
    ttt_batch_size = int(os.environ.get("TTT_BATCH_SIZE", 64))
    ttt_lora_enabled = bool(int(os.environ.get("TTT_LORA_ENABLED", "1")))
...
class CastedLinear(nn.Linear):
    ...
    _tanh_reparam: bool = False

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight
        # Tanh reparameterization: .weight stores latent P, effective W = tanh(P)
        if CastedLinear._tanh_reparam and w.ndim == 2 and min(w.shape) >= 64:
            w = torch.tanh(w)
        w = w.to(x.dtype)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if lora:
            # LoRA TTT path: per-token loss, no MTP, lm_head delta
            if self.tie_embeddings:
...
def quant_reg_loss(model: nn.Module, kure_lambda: float, r2_lambda: float) -> Tensor:
    """KURE kurtosis→1.8 + R2 margin reg for int6-friendly weights."""
    if kure_lambda <= 0 and r2_lambda <= 0:
    ...
    for name, p in model.named_parameters():
        ...
        if kure_lambda > 0:
            mu = pf.mean()
            var = pf.var(unbiased=False)
            kurt = ((pf - mu) ** 4).mean() / (var ** 2 + 1e-8)
            loss = loss + kure_lambda * (kurt - 1.8) ** 2
        if r2_lambda > 0:
            M = 2.0 * pf.std(unbiased=False)
            loss = loss + r2_lambda * (M + torch.clamp(pf.abs() - M, min=0).mean())
    return loss
...
def main() -> None:
    ...
    CastedLinear._tanh_reparam = args.tanh_reparam
    ...
    # LAWA-EMA: parallel float32 shadows for exponential moving average
    lawa_decays: list[float] = []
    lawa_shadows: dict[float, dict[str, Tensor]] = {}
    if args.lawa_enabled:
        lawa_decays = [0.995, 0.999, 0.9995]
        for d in lawa_decays:
            shadow = {}
            for name, p in base_model.named_parameters():
                if p.is_floating_point():
                    val = p.detach().float().clone()
                    if args.tanh_reparam and p.ndim == 2 and min(p.shape) >= 64:
                        val = torch.tanh(val)
                    shadow[name] = val
            lawa_shadows[d] = shadow
        log0(f"lawa:init decays={lawa_decays} shadow_params={len(shadow)}")
    ...
    while True:
        ...
        # LAWA-EMA: update parallel shadows after each optimizer step
        if lawa_shadows:
            with torch.no_grad():
                for d in lawa_decays:
                    for name, p in base_model.named_parameters():
                        if name in lawa_shadows[d]:
                            val = p.float()
                            if args.tanh_reparam and p.ndim == 2 and min(p.shape) >= 64:
                                val = torch.tanh(val)
                            lawa_shadows[d][name].mul_(d).add_(val, alpha=1 - d)
    ...
    # Disable tanh reparam for export — shadows already store effective weights.
    # If LAWA is not enabled, materialize tanh(P) into .weight directly.
    CastedLinear._tanh_reparam = False
    if args.tanh_reparam and not lawa_shadows:
        log0("tanh_reparam:materializing tanh(P) into weights for export")
        with torch.no_grad():
            for m in base_model.modules():
                if isinstance(m, CastedLinear) and m.weight.ndim == 2 and min(m.weight.shape) >= 64:
                    m.weight.data = torch.tanh(m.weight.data)

    # Apply LAWA-EMA: pick best decay via proxy eval, then apply
    if lawa_shadows:
        lawa_averaged = {d: {k: v.clone() for k, v in s.items()} for d, s in lawa_shadows.items()}
        best_bpb, best_decay = float('inf'), lawa_decays[0]
        for d in lawa_decays:
            sd = base_model.state_dict()
            for k, v in lawa_averaged[d].items():
                sd[k].copy_(v.to(device=sd[k].device, dtype=sd[k].dtype))
            base_model.load_state_dict(sd)
            _, proxy_bpb = eval_val(
                args, compiled_model, rank, world_size, device, grad_accum_steps,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            )
            log0(f"lawa:decay={d} proxy_bpb={proxy_bpb:.5f}")
            if proxy_bpb < best_bpb:
                best_bpb, best_decay = proxy_bpb, d
        # Apply winner
        sd = base_model.state_dict()
        for k, v in lawa_averaged[best_decay].items():
            sd[k].copy_(v.to(device=sd[k].device, dtype=sd[k].dtype))
        base_model.load_state_dict(sd)
        log0(f"lawa:selected decay={best_decay} proxy_bpb={best_bpb:.5f}")
        if distributed:
            for t in base_model.state_dict().values():
    ...
    # LoRA test-time training eval
    if args.ttt_lora_enabled:
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(
            f"final_ttt_lora val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} "
